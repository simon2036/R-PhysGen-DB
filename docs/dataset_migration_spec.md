...
本项目当前不使用 Alembic、Prisma Migrate、Django migrations 等传统数据库迁移框架。`data/index/*.duckdb` 是可重建发布产物，不是主存储；schema 或数据契约变化应优先通过 pipeline 重建 Parquet 与 DuckDB。
...
`data/gold/VERSION` 是当前构建的数据集版本来源，并且必须和 `data/gold/quality_report.json` 中的 `dataset_version` 保持一致。
...
每次改变数据契约、版本、核心输出或构建/验证规则时，都必须在 `docs/dataset_migrations/` 新增迁移记录。记录文件名格式：
...
当前 `data/gold/VERSION` 必须至少有一条 `review_status: applied` 或 `review_status: released` 的迁移记录。
...
- `docs/dataset_migrations/*.md`
- `data/gold/VERSION`
- `src/r_physgen_db/validate.py` 或相关验证模块
- `tests/` 中的 schema、版本、迁移或契约测试

如果变更影响 DuckDB 可查询表，仍然只更新 pipeline 的 DuckDB 重建逻辑；不要对 `data/index/*.duckdb` 做手工原地迁移。
