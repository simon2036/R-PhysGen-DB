...
- `data/raw/manual/active_learning_queue.csv`
- `data/raw/manual/active_learning_decision_log.csv`
...
Mixture fractions are not imputed. `data/raw/manual/mixture_fraction_curations.csv` may update existing mixture/component/basis rows only when the curation row includes source metadata. Unresolved rows, including `MIX_511A` when no traceable fraction source is present, remain unresolved.
