...
- The curated base inventory in `data/raw/manual/seed_catalog.csv` remains authoritative for direct manual edits; generated supplements are append-only build inputs.

## Property Governance Bundle Artifacts

The governed bundle intake writes the following contract-level artifacts:

- `methods/refrigerant_seed_database_20260422_property_governance_bundle.zip`
  Source bundle snapshot for the `2026-04-22` governance intake.
- `data/extensions/property_governance_20260422/extension_manifest.parquet`
  Manifest of mirrored bundle tables with row counts.
- `data/extensions/property_governance_20260422/tables/*.parquet`
  Fidelity-preserving mirror of every bundle table.
- `data/extensions/property_governance_20260422/mixture_core.parquet`
  Normalized mixture-level table derived from the mirrored bundle.
- `data/extensions/property_governance_20260422/mixture_component.parquet`
...
- `data/extensions/property_governance_20260422/tables/*.parquet` is the fidelity layer; normalized extension outputs do not replace it.
- `data/bronze/property_governance_20260422_unresolved_substances.parquet` may be empty, but it must still represent the post-curation unresolved state rather than the raw pre-curation state.

## Unresolved Bundle Curation Input

`data/raw/manual/property_governance_20260422_unresolved_curations.csv` is a controlled manual/external override input for unresolved governance-bundle substances.
...
`data/raw/manual/property_governance_20260422_canonical_review_decisions.csv` is a controlled manual adjudication input for already-selected canonical recommendations that should be removed from the open review queue without changing the selected canonical recommendation.
...
`data/raw/manual/property_governance_20260422_proxy_acceptance_rules.csv` is a controlled manual policy input for proxy-only canonical recommendations that may enter the strict ML-facing subset when no non-proxy candidate exists.
...
- Matching rows listed in `data/raw/manual/property_governance_20260422_canonical_review_decisions.csv` are excluded from this table, so it represents only open review work and may legitimately be empty after all current items are adjudicated.
- Matching rows admitted into `property_recommended_canonical_strict` through `data/raw/manual/property_governance_20260422_proxy_acceptance_rules.csv` are also excluded from this table, so proxy-only policy acceptance closes those queue items without changing the selected canonical recommendation.
...
`data/raw/manual/property_governance_20260422_unresolved_curations.csv` is not part of this observation merge. It is handled earlier as an identity-resolution override input for the governance bundle crosswalk.
